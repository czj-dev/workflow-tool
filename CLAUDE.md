...
- **LLM 域**：`command.llm{system, prompt}` 形态由 `runner.LLMRunner` 处理（对标 ADBRunner 的一等形态，不再寄生在 shell/stdin/stream 三个字段上）。`LLMRunner.Run` 直接 `exec.Command` 构造子进程 argv：CLI 名取 `config.yaml` 的 `LLM_CLI`（默认 `ducc`）+ 固定 flags（`-p --output-format=stream-json --verbose --thinking enabled`）+ `system` 对应 param 值作为 `--append-system-prompt` 的**独立 argv**（不经 shell 字符串，多行/引号/`$` 零风险）；`prompt` 对应 param 值写入 `cmd.Stdin`。stdout 按 stream-json 解析（`llm.go` 的 `parseLLMLine`/`recordStructuredFields`），只把 assistant 的 text / thinking 增量 emit 为 `"llm"` / `"llm-thinking"`。`command.llm.skills` 声明绑定的 skill 在 Build 期由 `actionrun.SkillRouter` 按「CLI 路由表 × 作用域（skills/ 根=project 随 cwd、skills/user/=user 落 home）」同步到 Agent 目录（源为准比对覆盖，足迹记 exe 同级 skills.synced.json），报告行由 LLMRunner.Run 开头 emit——文件即注册，零 prompt 拼接。
