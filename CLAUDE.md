...
前端（`frontend/src/`，React 19 + TS + Vite + tailwind4 + base-ui/shadcn）：`ActionRunnerProvider` 是唯一状态中枢，`ListActions` 拉动作、`Events.On` 订阅输出事件、`view` 在 output/form/global/llm/logcat/workflow/fragments 间切。切 `llm` 视图的判定是 `action.llm != null`（`ActionItem.LLM` 由后端按 `command.llm` 是否非空注入），非 action.stream 值。`LlmForm.tsx` 是 `command.llm` 动作专用表单：`llm.promptParam` 指向的 param 是主位大号 textarea，`llm.systemParam` 指向的 param 折叠进「角色设定」（默认收起）。`DeviceSelector`（挂 `AppSidebar` SidebarHeader）调 `ListDevices`/`SetActiveDevice` 选激活设备，后端是激活 serial 唯一真相、运行 adb 动作时自动注入 `${ADB_SERIAL}`。binding 从 `frontend/bindings/`（`wails3 generate bindings` 产物，ES module）导入。UI 优先复用 `components/ui/` 下的 shadcn 原子（Badge / IconButton 等），避免手写重复样式。
